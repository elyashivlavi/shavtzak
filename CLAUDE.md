...
- **Do analysis & publishing at the DB/server layer, not via the UI** (user preference). For one-off admin operations (generate, publish, re-publish, verify fairness/night distribution, swaps), use a temporary admin-gated `doGet` endpoint (`?action=…&key=<admin_password>`), run it via `curl`, then remove it — don't wire one-offs into `Index.html`. Note: Apps Script `/exec` 302-redirects and `curl -L` re-runs the GET, so make such endpoints **idempotent** (a second run must not double-apply).
