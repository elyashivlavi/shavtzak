...
- **Manually fixing / re-planning a published schedule (learned):** when the user reports a rule violation and wants a *minimal* fix (not a full regen):
  ...
  5. **Write path = temporary admin-gated `doGet` branch** (`?fix…=<admin_password>`), looking soldier **names→ids** from the sheet, each swap wrapped in `try/catch` so it's **idempotent** (the `-L` 302 re-run skips already-applied swaps that now throw "not in positions"). `curl` it, **verify via a fresh export** (re-check consecutive + `קלע`-patrol), then **remove the branch and redeploy**. Never leave a data-mutating GET endpoint deployed.
  6. Blocks that have elapsed can't be edited (`blockElapsed_` guard) — fix future dates, not today's/past.
