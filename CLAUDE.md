...
- **Do analysis & publishing at the DB/server layer, not via the UI** (user preference).
  ...
  - **Fallback (temp endpoint):** if direct write is ever unavailable, a **temporary idempotent admin-gated `doGet` branch** reached via the head **`/dev` URL** after a plain `clasp push` (owner opens it in the browser) — **not** `clasp redeploy`+`/exec` (that bumps the permanent version and forces a commit). Remove the branch afterward with another `clasp push`.
