...
- For the selected day, **three titled cards**:
  ...
  - **פטרול (Patrol)** — the patrol soldiers split into **two labeled sets — ערב then בוקר** — matching the sheet's `slot` (evening/morning). Each soldier appears once per set.
