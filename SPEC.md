...
| `standby` | boolean | True for guards. On-call is derived **per 12h window** from each shift's start time: a guard is on call in a window only if he has a shift in it (usually both → 24h; a single-slot manual swap leaves him on call 12h and on patrol the other 12h). See §4. |
...
- **Standby (on-call) is per 12-hour window.** The 24h block splits into two windows — **morning** (00:00–12:00, shifts whose start hour is before the anchor) and **evening** (12:00–00:00, shifts from the anchor on). A guard is on standby for a window **only if he holds a guard shift in that window**. Since a normally-generated guard does one shift in each window (`[g0…]` round-robin puts each guard on slot `k` and slot `k+4`), he is on standby for both windows → effectively 24h. But when only **one** of a guard's two shifts is replaced manually (board single-slot swap), he is on standby for just that one 12h window; in the other window he is on patrol (and whoever took the other slot is on standby there and on patrol in the first). The `standby` column stays `true` on each stored guard row — the window split is derived from each shift's start time.
- **Patrol is derived, not stored, and defined per 12-hour window** (rather than being written out as explicit rows): for each window, patrol = every **present, active** soldier who does **not** hold a guard shift in that window. Concretely — the **morning** patrol (00:00–12:00) and **evening** patrol (12:00–00:00) are computed independently. A guard on both windows appears on neither patrol; a guard on only one window appears on the other window's patrol; every other present soldier appears on both. No explicit DB entries are needed — patrol is computed from the guard rows + the soldiers' presence windows wherever it is shown.
...
- For the selected day, **three titled cards**:
  ...
  - **כוננות (Standby)** — the on-call soldiers of the selected calendar day, shown as **two sets computed independently per 12h window**: **עד 12:00** = the previous block's **morning-window** guards (their 00:00–12:00 shifts, which fall on this day), **מ-12:00** = this block's **evening-window** guards (12:00–00:00). A calendar day is covered by two consecutive blocks, and each set lists only guards who actually hold a shift in that window — so a soldier whose single shift was swapped out of a window appears on-call in the other window and on **patrol** here instead (mirrors the פטרול card).
...
- **On-call (כוננות):** on-call = the block's guards **split per 12h window** (a guard is on call only in the window(s) he has a shift in), so the כוננות card is **read-only** (shown to everyone, not editable even in admin) — it's derived from the עמדה shifts and updates automatically when the guard slots change.
